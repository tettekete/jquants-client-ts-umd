/*
# 投資部門別情報(/markets/trades_spec) API Sample
...
```sh
$ cd example
$ ts-node api-marketsTradesSpec.ts
```
...
(async ()=>{
	...
	const r = await jqc.marketsTradesSpec(
		{
			section: 'TSEPrime'
		}
	);
	...
	if( r.ok )
	{
		...
		// so you can access properties like r.data.trades_spec[0].EndDate
		...
		//
		// import {isMarketsTradesSpecResponse} from '../src/type-guard';
		// if( r.ok && isMarketsTradesSpecResponse( r.data ) )
		// {
		// 	console.log( r.data.trades_spec[0].EndDate );
		// }
	}
	else
	{
