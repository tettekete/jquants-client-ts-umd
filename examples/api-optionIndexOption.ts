/*
# 日経225オプション四本値(/option/index_option) API Sample
...
```sh
$ cd example
$ ts-node api-optionIndexOption.ts
```
...
(async ()=>{
	...
	const r = await jqc.optionIndexOption(
		{
			date: '2024-10-23'
		}
	);
	...
	{
		// r.data is inferred as OptionIndexOptionResponse,
		// so you can access properties like r.data.index_option[0].Code
		...
		//
		// import {isOptionIndexOptionResponse} from '../src/type-guard';
		// if( r.ok && isOptionIndexOptionResponse( r.data ) )
		// {
		// 	console.log( r.data.index_option[0].Code );
		// }
	}
	else
	{
